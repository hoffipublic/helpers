 ...
 * @param <V> Hash Value
 */
public class HashMapCloneableForMeta<K, V, MV> extends HashMap<K, V> implements MapCloneableForMeta<K, V, MV> {

    private static final long serialVersionUID = 2021324468361177494L;

    public HashMapCloneableForMeta() {
        super();
    }

    HashMapCloneableForMeta(int initialCapacity) {
        super(initialCapacity);
    }

    HashMapCloneableForMeta(int initialCapacity, float loadFactor) {
        super(initialCapacity, loadFactor);
    }

    HashMapCloneableForMeta(Map<? extends K, ? extends V> m) {
        super(m);
    }
